...
const AIMoodScreen = ({ navigation }) => {
  ...
  // Функция для генерации аффирмации через AI сервис
  const generateAffirmation = async (mood) => {
    ...
    try {
      // Используем AI сервис (текущая версия: моки, будущая: LLM API)
      const generatedText = await generateAI(mood);
      setAffirmation(generatedText);
    } catch (error) {
      console.error('Error generating affirmation:', error);
      setAffirmation('Произошла ошибка при генерации аффирмации. Попробуйте ещё раз.');
    } finally {
